...
pub(crate) fn declared_primary_key_name(
    ctx: &ConvertContext,
    collection: &str,
...
///
/// Identity minting then runs on the same declared name. A present declared
/// key content-addresses a surrogate via [`assign_for_pk`]. With no declared
/// key, minting falls back to the resolved `primary_key` argument. An
/// auto-`_rowid` pk or a missing/null key mints a fresh surrogate instead.
/// The two steps are one call so no caller can mint an identity without the
/// NOT NULL check running first.
pub(crate) fn resolve_doc_identity(
    ctx: &ConvertContext,
    collection: &str,
    primary_key: Option<&str>,
    row: &[(String, SqlValue)],
) -> crate::Result<(String, Surrogate)> {
    // `_rowid` carries no declaration: skip the catalog read entirely and
    // mint a surrogate below.
    let declared = if is_auto_rowid_pk(primary_key) {
        None
    } else {
        declared_primary_key_name(ctx, collection)?
    };
    resolve_doc_identity_with_declared(ctx, collection, primary_key, declared.as_deref(), row)
}

/// Core of [`resolve_doc_identity`], given the declared PK name already
/// resolved by the caller instead of reading it from the catalog here.
///
/// [`columnar_row_surrogates`] resolves `declared` once for the whole
/// statement and calls this directly for every row, so a multi-row INSERT
/// hits the catalog once rather than once per row.
pub(super) fn resolve_doc_identity_with_declared(
    ctx: &ConvertContext,
    collection: &str,
    primary_key: Option<&str>,
    declared: Option<&str>,
    ...
    let mint_key = declared.or(primary_key);
...
pub(crate) fn columnar_row_surrogates(
    ctx: &ConvertContext,
    collection: &str,
    ...
    // `_rowid` carries no declaration, matching `resolve_doc_identity`.
    let declared = if is_auto_rowid_pk(primary_key) {
        None
    } else {
        declared_pk
