...
pub(super) async fn resolve_update_period_values(
    ...
    // `ResolvedTargets` dedupes on `(target, value)` itself, so the old and
    // new images resolve through the same call whether or not they carry the
    // same period value.
    ...
    for row in [&old_row, &new_row] {
        ...
        // No reference row names this period — the resolve leaves it
        // unresolved so `check_period_lock` reports the unknown-period
        // refusal for it.
        resolved
