...
use super::super::catalog::{StoredUser, SystemCatalog};
...
impl CredentialStore {
    ...
    //
    // Three methods power the `CatalogEntry::PutUser` /
    // `DeactivateUser` pipeline. The pgwire handler on the leader
    // calls `prepare_user` to build a complete `StoredUser` without
    // touching the in-memory map or redb; the proposer ships it
    // through raft; every node's applier then calls
    // `install_replicated_user` to upsert the in-memory record and
    // (separately) `catalog.put_user` to write the redb.
    //
    // Password hashing + scram salt generation must happen on the
    // leader because followers cannot reproduce the same random salt.
    // `install_replicated_user` accepts the leader's fully-computed
    // `StoredUser` verbatim and bumps `next_user_id` so the follower
    // stays ahead of any replicated IDs if leadership changes later.
    ...
    pub fn prepare_user(
        ...
        // Uniqueness check against the in-memory cache so the
        // leader rejects duplicates before touching raft. The
        // cache is authoritatively replicated via the applier, so
        // every node agrees on membership.
        {
    ...
    /// Never errors — a poisoned lock falls through to in-place
    /// recovery so a single bad user write doesn't stall raft.
    pub fn install_replicated_user(&self, stored: &StoredUser) {
        ...
        // Bump local `next_user_id` to stay ahead of the replicated
        // value — prevents collisions if leadership changes and
        // this node starts allocating its own IDs.
