...
//! WAL replay for the vector-primary `DELETE` / `UPDATE` records.
//!
//! Both records route through the live handlers, so the rebuilt node,
//! bitmap entries, and sidecar match what the live write produced. Both are
...
use crate::control::server::wal_dispatch::{VectorDirectDeleteRecord, VectorDirectUpdateRecord};
