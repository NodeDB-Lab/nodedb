...
pub(super) fn add(stored: &[u8], delta: &str) -> Result<(f64, Vec<u8>), AtomicError> {
    let text = std::str::from_utf8(stored)
        .ok()
        .filter(|text| is_decimal_number(text))
        .ok_or(AtomicError::Counter(CounterFault::NotAFloat))?;
        ...
        && let Some(sum) = base.checked_add(step)
    {
        ...
        let value = sum
            .to_f64()
            .ok_or(AtomicError::Counter(CounterFault::NonFinite))?;
        return Ok((value, sum.to_string().into_bytes()));
    }
    let base: f64 = text
        .parse()
        .map_err(|_| AtomicError::Counter(CounterFault::NotAFloat))?;
...
pub(super) fn fresh(delta: &str) -> Result<(f64, Vec<u8>), AtomicError> {
    add(b"0", delta)
}
...
/// `Counter(NonFinite)`.
pub fn delta_to_f64(delta: &str) -> Result<f64, AtomicError> {
    if !is_decimal_number(delta) {
        return Err(AtomicError::Counter(CounterFault::NotAFloat));
    }
    let value: f64 = delta
        .parse()
        .map_err(|_| AtomicError::Counter(CounterFault::NotAFloat))?;
