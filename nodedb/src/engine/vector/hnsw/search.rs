...
impl HnswIndex {
    ...
    /// Must be >= k. Typical production values: ef = 2*k to 10*k.
    ...
    pub fn search_filtered(
        ...
        // Beam search at layer 0 with filter applied to result collection.
    ...
    ///
    /// This is the entry point used by the CoreLoop when `filter_bitmap` is
    /// provided in the `VectorSearch` physical plan.
...
///
/// When `filter` is Some, only nodes present in the bitmap are added to the
/// result set. All nodes are still traversed for navigation quality.
///
/// Implements the core search-layer procedure from Algorithm 2
/// (Malkov & Yashunin, 2018) with the filtered extension.
pub(super) fn search_layer(
    ...
    // candidates: min-heap (closest first) — next nodes to explore.
    let mut candidates: BinaryHeap<Reverse<Candidate>> = BinaryHeap::new();
    candidates.push(Reverse(ep_candidate));

    // results: max-heap (furthest first) — best ef results found so far.
    // The top of the heap is the worst result, enabling O(1) pruning.
...
mod tests {
    ...
    fn search_finds_exact_match() {
        ...
        // Search for the exact vector of node 25.
    ...
    #[test]
    fn search_recall_at_100() {
        // Build index with 500 3D vectors, search for top-10.
        ...
        let mut truth: Vec<(u32, f32)> = (0..500)
            .map(|i| {
                ...
                let d = crate::engine::vector::distance::l2_squared(&query, v);
                (i, d)
            })
            .collect();
    ...
    #[test]
    fn filtered_search_excludes_unfiltered() {
        let idx = build_index(100, 3);
        ...
        let results = idx.search_filtered(&[0.0, 0.0, 0.0], 10, 64, &filter);

        for result in &results {
            assert!(
                result.id % 2 == 0,
                "filtered search returned odd node {}",
                result.id
            );
        }
    }
    ...
    #[test]
    fn filtered_search_empty_filter_returns_empty() {
        let idx = build_index(50, 3);
    ...
    fn search_high_dimensional() {
        // 128-dim vectors (typical small embedding).
        let mut idx = HnswIndex::with_seed(
            128,
            HnswParams {
                m: 16,
